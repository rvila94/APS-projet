...
let rec eval_stat s env flux mem =
  match s with
      ASTEcho(e)          ->
    ...
    (*| ASTCall(fname, es)  -> 
              begin match (eval_expr es env mem) with *)
...
and eval_cmds cmds env flux mem =
  ...
  | ASTStat2(s, cs) -> let (_, new_mem) = (eval_stat s env flux mem) in
                eval_cmds cs env flux new_mem
